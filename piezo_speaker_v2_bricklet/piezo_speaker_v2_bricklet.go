...
func (device *PiezoSpeakerV2Bricklet) GetBeep() (frequency uint16, volume uint8, duration BeepDuration, durationRemaining uint32, err error) {
    ...
    if len(resultBytes) > 0 {
        ...
        header.FillFromBytes(resultBytes)
        if header.ErrorCode != 0 {
            return frequency, volume, duration, durationRemaining, BrickletError(header.ErrorCode)
        }
...
func (device *PiezoSpeakerV2Bricklet) GetAlarm() (startFrequency uint16, endFrequency uint16, stepSize uint16, stepDelay uint16, volume uint8, duration AlarmDuration, durationRemaining uint32, currentFrequency uint16, err error) {
    ...
    if len(resultBytes) > 0 {
        ...
        header.FillFromBytes(resultBytes)
        if header.ErrorCode != 0 {
            return startFrequency, endFrequency, stepSize, stepDelay, volume, duration, durationRemaining, currentFrequency, BrickletError(header.ErrorCode)
        }
