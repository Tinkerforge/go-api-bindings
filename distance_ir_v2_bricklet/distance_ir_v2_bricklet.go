...
func (device *DistanceIRV2Bricklet) GetAnalogValue() (analogValue uint32, err error) {
	...
	if len(resultBytes) > 0 {
		...
		if header.Length != 12 {
			return analogValue, fmt.Errorf("Received packet of unexpected size %d, instead of %d", header.Length, 12)
		}
...
func (device *DistanceIRV2Bricklet) GetMovingAverageConfiguration() (movingAverageLength uint16, err error) {
	...
	if len(resultBytes) > 0 {
		...
		if header.Length != 10 {
			return movingAverageLength, fmt.Errorf("Received packet of unexpected size %d, instead of %d", header.Length, 10)
		}
...
func (device *DistanceIRV2Bricklet) GetSensorType() (sensor SensorType, err error) {
	...
	if len(resultBytes) > 0 {
		...
		if header.Length != 9 {
			return sensor, fmt.Errorf("Received packet of unexpected size %d, instead of %d", header.Length, 9)
		}
