...
func (device *LCD20x4Bricklet) DeregisterButtonPressedCallback(callbackID uint64) {
    device.device.DeregisterCallback(uint8(FunctionCallbackButtonPressed), callbackID)
}
...
func (device *LCD20x4Bricklet) DeregisterButtonReleasedCallback(callbackID uint64) {
    device.device.DeregisterCallback(uint8(FunctionCallbackButtonReleased), callbackID)
}
...
func (device *LCD20x4Bricklet) GetDefaultText(line uint8) (text string, err error) {    
    ...
    if len(resultBytes) > 0 {
        ...
        header.FillFromBytes(resultBytes)
        if header.ErrorCode != 0 {
            return text, BrickletError(header.ErrorCode)
        }
