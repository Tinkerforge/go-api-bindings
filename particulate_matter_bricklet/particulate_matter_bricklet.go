...
func (device *ParticulateMatterBricklet) GetPMConcentration() (pm10 uint16, pm25 uint16, pm100 uint16, err error) {
	...
	if len(resultBytes) > 0 {
		...
		if header.Length != 14 {
			return pm10, pm25, pm100, fmt.Errorf("Received packet of unexpected size %d, instead of %d", header.Length, 14)
		}
...
func (device *ParticulateMatterBricklet) GetPMCount() (greater03um uint16, greater05um uint16, greater10um uint16, greater25um uint16, greater50um uint16, greater100um uint16, err error) {
	...
	if len(resultBytes) > 0 {
		...
		if header.Length != 20 {
			return greater03um, greater05um, greater10um, greater25um, greater50um, greater100um, fmt.Errorf("Received packet of unexpected size %d, instead of %d", header.Length, 20)
		}
...
func (device *ParticulateMatterBricklet) GetSensorInfo() (sensorVersion uint8, lastErrorCode uint8, framingErrorCount uint8, checksumErrorCount uint8, err error) {
	...
	if len(resultBytes) > 0 {
		...
		if header.Length != 12 {
			return sensorVersion, lastErrorCode, framingErrorCount, checksumErrorCount, fmt.Errorf("Received packet of unexpected size %d, instead of %d", header.Length, 12)
		}
