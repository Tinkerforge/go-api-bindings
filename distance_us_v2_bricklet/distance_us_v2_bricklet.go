...
func (device *DistanceUSV2Bricklet) GetResponseExpected(functionID Function) (bool, error) {
...
// for this purpose. If this flag is disabled for a setter function then no response is sent
// and errors are silently ignored, because they cannot be detected.
func (device *DistanceUSV2Bricklet) SetResponseExpected(functionID Function, responseExpected bool) error {
	return device.device.SetResponseExpected(uint8(functionID), responseExpected)
}

// Changes the response expected flag for all setter and callback configuration functions of this device at once.
func (device *DistanceUSV2Bricklet) SetResponseExpectedAll(responseExpected bool) {
	device.device.SetResponseExpectedAll(responseExpected)
}

// Returns the version of the API definition (major, minor, revision) implemented by this API bindings. This is neither the release version of this API bindings nor does it tell you anything about the represented Brick or Bricklet.
func (device *DistanceUSV2Bricklet) GetAPIVersion() [3]uint8 {
...
func (device *DistanceUSV2Bricklet) DeregisterDistanceCallback(registrationId uint64) {
...
func (device *DistanceUSV2Bricklet) GetUpdateRate() (updateRate UpdateRate, err error) {
	...
	if len(resultBytes) > 0 {
		...
		if header.Length != 9 {
			return updateRate, fmt.Errorf("Received packet of unexpected size %d, instead of %d", header.Length, 9)
		}

		if header.ErrorCode != 0 {
			return updateRate, DeviceError(header.ErrorCode)
		}

		resultBuf := bytes.NewBuffer(resultBytes[8:])
		binary.Read(resultBuf, binary.LittleEndian, &updateRate)
